...
// Funções auxiliares para análise semântica

void declarar_variavel_global(char *nome, tipo_dado_t tipo, int linha) {
    // Verifica dupla declaração no escopo atual
    if (buscar_simbolo_escopo_atual(pilha_tabelas, nome) != NULL) {
        fprintf(stderr, "ERRO SEMÂNTICO na linha %d: variável '%s' já foi declarada.\n", linha, nome);
        exit(ERR_DECLARED);
    }
...
entrada_tabela_t* verificar_chamada_funcao(char *nome, int linha) {
    ...
    if (entrada == NULL) {
        fprintf(stderr, "ERRO SEMÂNTICO na linha %d: função '%s' não foi declarada.\n", linha, nome);
        exit(ERR_UNDECLARED);
    }
    
    if (entrada->natureza != NAT_FUNCAO) {
        fprintf(stderr, "ERRO SEMÂNTICO na linha %d: '%s' é uma variável e não pode ser chamada como função.\n",
                linha, nome);
        exit(ERR_VARIABLE);
    }
...
int contar_argumentos(asd_tree_t *args) {
    ...
    int count = 1;
    for (int i = 0; i < args->number_of_children; i++) {
        count += contar_argumentos(args->children[i]);
    }
    return count;
}
...
void verificar_argumentos_funcao(entrada_tabela_t *func, asd_tree_t *args, int linha) {
    ...
    if (num_args < num_params) {
        fprintf(stderr, "ERRO SEMÂNTICO na linha %d: chamada de função '%s' com menos argumentos que o esperado.\n",
                linha, func->chave);
        exit(ERR_MISSING_ARGS);
    }
    
    if (num_args > num_params) {
        fprintf(stderr, "ERRO SEMÂNTICO na linha %d: chamada de função '%s' com mais argumentos que o esperado.\n",
                linha, func->chave);
    ...
    while (p != NULL && arg != NULL) {
        // Pega o primeiro argumento que não é uma lista
        asd_tree_t *arg_atual = arg;
        while (arg_atual->number_of_children > 0 && 
               arg_atual->children[0]->number_of_children > 0) {
            arg_atual = arg_atual->children[0];
        }
        
        if (arg_atual->tipo != p->tipo && arg_atual->tipo != TIPO_NAO_DEFINIDO) {
            fprintf(stderr, "ERRO SEMÂNTICO na linha %d: argumento %d da função '%s' com tipo incompatível (esperado %s, recebido %s).\n",
                    linha, pos, func->chave, tipo_para_string(p->tipo), tipo_para_string(arg_atual->tipo));
