...
char* int_to_str(int num, char* prefix)
{
	char* ret = calloc(100, sizeof(char));
	
	//this function will assume that the caller is not a jackass who uses a huge prefix for literally no reason
	
	if (prefix != NULL)
		snprintf(ret, 100, "%s%d", prefix, num);
	else
		snprintf(ret, 100, "%d", num);
...
tabela_simbolos_t* criar_tabela() {
    ...
    tabela->funcao = NULL;
    tabela->deslocamento_acumulado = 0;
    return tabela;
}
...
void declarar_variavel_global(tabela_simbolos_t *pilha, valor_t *token, tipo_dado_t tipo, int linha) {
    ...
    //Para a etapa 5:
    entrada_tabela_t *entrada = pilha->primeiro;
    entrada->base = strdup("rbss");
    //Calcula o offset
    char buffer_offset[16];
    sprintf(buffer_offset, "%d", pilha->deslocamento_acumulado);
    entrada->deslocamento = strdup(buffer_offset);
    pilha->deslocamento_acumulado += 4;
}
...
void liberar_tabela(tabela_simbolos_t *tabela) {
    ...
    while (atual != NULL) {
        ...
        if (temp->base) free(temp->base);
        if (temp->deslocamento) free(temp->deslocamento);
