...
void iloc_expressao_binaria(asd_tree_t *src, opcode_t opcode, asd_tree_t *op1, asd_tree_t *op2);
void iloc_expressao_unaria(asd_tree_t *src, char expr_type, asd_tree_t *op);
void iloc_load_literal(asd_tree_t *src, char* literal);
void iloc_load_variavel(asd_tree_t *src, tabela_simbolos_t *stack, char* key);
void iloc_store_variavel(asd_tree_t *src, tabela_simbolos_t *stack, char* key, asd_tree_t *expr);
void iloc_if(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *block);
void iloc_if_else(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *true_block, asd_tree_t *false_block);
void iloc_while(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *block);
  ...
  | elemento ',' lista
    {
        if ($1 != NULL) {
            ...
            if ($3 != NULL) {
                asd_add_child($$, $3);
                append_lista_codigo($$->codigo, $3->codigo);
            }
        } else {
            $$ = $3;
        }
  ...
  : TK_VAR TK_ID TK_ATRIB tipo
    {
        ...
        //iloc_store_variavel($$, pilha_tabelas, $2->lexema, $4);
  ...
  : cabecalho corpo
    {
        ...
        if ($2 != NULL) {
            asd_add_child($1, $2);
            append_lista_codigo($$->codigo, $2->codigo);
        }
  ...
  : TK_VAR TK_ID TK_ATRIB tipo
    {
        ...
        //iloc_store_variavel($$, pilha_tabelas, $2->lexema, )
        $$ = NULL;
        free(token_identificador->lexema);
        free(token_identificador);
    }
  | TK_VAR TK_ID TK_ATRIB tipo TK_COM literal
    {
        ...
        iloc_store_variavel($$, pilha_tabelas, $2->lexema, $6);
  ...
  : TK_ID TK_ATRIB expressao
    {
        ...
        iloc_store_variavel($$, pilha_tabelas, $1->lexema, $3);
  ...
  : expressao_and '|' expressao_or
    {
      ...
      iloc_expressao_binaria($$, ILOC_OR, $1, $3);
    }
  | expressao_and { $$ = $1; }
  ;
  ...
  : expressao_igual_desigual '&' expressao_and
    {
      ...
      no_operador->tipo = inferir_tipo_binario(no_esquerdo->tipo, no_direito->tipo);
      $$ = no_operador;
      iloc_expressao_binaria($$, ILOC_AND, $1, $3);
    }
  ...
  : expressao_relacional TK_OC_EQ expressao_igual_desigual
    {
      ...
      iloc_expressao_binaria($$, ILOC_CMP_EQ, $1, $3);
    }
  | expressao_relacional TK_OC_NE expressao_igual_desigual
    {
      ...
      no_operador->tipo = inferir_tipo_binario(no_esquerdo->tipo, no_direito->tipo);
      $$ = no_operador;
      iloc_expressao_binaria($$, ILOC_CMP_NE, $1, $3);
    }
  ...
  : expressao_soma_subtracao '<'      expressao_relacional
    {
      ...
      no_operador->tipo = inferir_tipo_binario(no_esquerdo->tipo, no_direito->tipo);
      $$ = no_operador;
      iloc_expressao_binaria($$, ILOC_CMP_LT, $1, $3);
    }
  | expressao_soma_subtracao '>'      expressao_relacional
    {
      asd_tree_t *no_esquerdo = $1;
      ...
      iloc_expressao_binaria($$, ILOC_CMP_GT, $1, $3);
    }
  | expressao_soma_subtracao TK_OC_LE expressao_relacional
    {
      ...
      no_operador->tipo = inferir_tipo_binario(no_esquerdo->tipo, no_direito->tipo);
      $$ = no_operador;
      iloc_expressao_binaria($$, ILOC_CMP_LE, $1, $3);
    }
  | expressao_soma_subtracao TK_OC_GE expressao_relacional
    {
      ...
      no_operador->tipo = inferir_tipo_binario(no_esquerdo->tipo, no_direito->tipo);
      $$ = no_operador;
      iloc_expressao_binaria($$, ILOC_CMP_GE, $1, $3);
    }
  ...
  | expressao_mult_div_mod '-' expressao_soma_subtracao
    {
      ...
      iloc_expressao_binaria($$, ILOC_SUB, $1, $3);
    }
...
expressao_unitario
  : '+' expressao_unitario { $$ = asd_new("+"); asd_add_child($$, $2); $$->tipo = $2->tipo; iloc_expressao_unaria($$, '+', $2);}
  | '-' expressao_unitario { $$ = asd_new("-"); asd_add_child($$, $2); $$->tipo = $2->tipo; iloc_expressao_unaria($$, '-', $2);}
  | '!' expressao_unitario { $$ = asd_new("!"); asd_add_child($$, $2); $$->tipo = $2->tipo; iloc_expressao_unaria($$, '!', $2);}
  ...
  : TK_ID
    {
        ...
        iloc_load_variavel($$, pilha_tabelas, $1->lexema);
...
void iloc_expressao_binaria(asd_tree_t *src, opcode_t opcode, asd_tree_t *op1, asd_tree_t *op2)
{
	src->temp = novo_temporario();
	
	append_lista_codigo(src->codigo, op1->codigo);
	append_lista_codigo(src->codigo, op2->codigo);
	inserir_codigo(src->codigo, criar_iloc(opcode, op1->temp, op2->temp, src->temp));
}

void iloc_expressao_unaria(asd_tree_t *src, char expr_type, asd_tree_t *op)
{
	code_t* new_code = cria_codigo_vazio();
	
	// somente 3 expressões unárias:
	// - '+' =  não faz nada...?
	// - '-' = negação aritmética
	// - '!' = negação lógica
	if (expr_type == '-'){
		// negação aritmética
		src->temp = novo_temporario();
		inserir_codigo(new_code, criar_iloc(ILOC_RSUBI, op->temp, "0", src->temp)); // src recebe (0 - op), aka -op
	}
	else if (expr_type == '!'){
		// negação lógica
		// não existe instrução 'not', pelo menos não no ilocsim.py dado
		// e não existem instruções CMP com imediatos
		// portanto, usar duas instruções
		src->temp = novo_temporario();
		char* zero_temp = novo_temporario();
		inserir_codigo(new_code, criar_iloc(ILOC_LOADI, "0", zero_temp, NULL));
		inserir_codigo(new_code, criar_iloc(ILOC_CMP_EQ, op->temp, zero_temp, src->temp)); // src recebe (op == false), aka !op
		free(zero_temp);
	}
	else{
		// literally nothing?
		//src->temp = strdup(op->temp);
		//if (src->temp == NULL) memory_fail();
	}
	
	append_lista_codigo(src->codigo, op->codigo);
	append_lista_codigo(src->codigo, new_code);
	destrutor_codigo(new_code);
}
...
void iloc_load_literal(asd_tree_t *src, char* literal)
{
	...
	inserir_codigo(src->codigo, criar_iloc(ILOC_LOADI, literal, src->temp, NULL));
}

void iloc_load_varivel(asd_tree_t *src, tabela_simbolos_t *stack, char* chave)
{
	src->temp = novo_temporario();
	entrada_tabela_t* simbolo = buscar_simbolo(stack, chave);
	
	inserir_codigo(src->codigo, criar_iloc(ILOC_LOADAI, simbolo->base, simbolo->deslocamento, src->temp));
}

void iloc_store_variavel(asd_tree_t *src, tabela_simbolos_t *stack, char* chave, asd_tree_t *expr)
{
	entrada_tabela_t* simbolo = buscar_simbolo(stack, chave);
	
	append_lista_codigo(src->codigo, expr->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_STOREAI, expr->temp, simbolo->base, simbolo->deslocamento));
}

void iloc_if(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *block)
{
	char *true_label, *exit_label;
	true_label = nova_label();
	exit_label = nova_label();
	
	// avaliação da expr + cbr
	append_lista_codigo(src->codigo, expr->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_CBR, expr->temp, true_label, exit_label));
	
	// se true, executa o bloco if
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, true_label, NULL, NULL));
	if (block != NULL) // para poder funcionar com blocos vazios []
		append_lista_codigo(src->codigo, block->codigo);
	
	// se false, pula o bloco if
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, exit_label, NULL, NULL));
	
	// libera memória das labels
	free(true_label);
	free(exit_label);
}
...
void iloc_if_else(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *true_block, asd_tree_t *false_block)
{
	char *true_label, *false_label, *exit_label;
	true_label = nova_label();
	false_label = nova_label();
	exit_label = nova_label();
	
	// avaliação da expr + cbr
	append_lista_codigo(src->codigo, expr->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_CBR, expr->temp, true_label, false_label));
	
	// se true, executa o bloco if e pula o bloco else
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, true_label, NULL, NULL));
	if (true_block != NULL) // para poder funcionar com blocos vazios []
		append_lista_codigo(src->codigo, true_block->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_JUMPI, exit_label, NULL, NULL));
	
	// se false, pula o bloco if e executa o bloco else
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, false_label, NULL, NULL));
	if (false_block != NULL) // para poder funcionar com blocos vazios []
		append_lista_codigo(src->codigo, false_block->codigo);
...
void iloc_while(asd_tree_t *src, asd_tree_t *expr, asd_tree_t *block)
{
	char *expr_label, *loop_label, *exit_label;
	expr_label = nova_label();
	loop_label = nova_label();
	exit_label = nova_label();
	
	// avaliação da expr + cbr
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, expr_label, NULL, NULL));
	append_lista_codigo(src->codigo, expr->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_CBR, expr->temp, loop_label, exit_label));
	
	// bloco do laço + jump para avaliação da expr
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, loop_label, NULL, NULL));
	if (block != NULL) // para poder funcionar com blocos vazios []
		append_lista_codigo(src->codigo, block->codigo);
	inserir_codigo(src->codigo, criar_iloc(ILOC_JUMPI, expr_label, NULL, NULL));
	
	// label de saída do laço
	inserir_codigo(src->codigo, criar_iloc(ILOC_LABEL, exit_label, NULL, NULL));
...
void iloc_load_variavel(asd_tree_t *src, tabela_simbolos_t *pilha, char* chave)
{
	src->temp = novo_temporario();
	entrada_tabela_t* simbolo = buscar_simbolo(pilha, chave);
